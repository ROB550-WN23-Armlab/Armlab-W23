...
# ******************************** PUBLIC FUNCTIONS IF CONTROLLING AN INTERBOTIX ARM OR TURRET ********************************
    ...
    ### @brief Helper function to get the position PID parameters for the specified joint
    ### @param joint_name - name of joint to read gains from
    def get_joint_position_pid_params(self, joint_name):
        p_gain = self.srv_get_register(cmd=RegisterValuesRequest.SINGLE_MOTOR, motor_name=joint_name, addr_name="Position_P_Gain").values[0]
        d_gain = self.srv_get_register(cmd=RegisterValuesRequest.SINGLE_MOTOR, motor_name=joint_name, addr_name="Position_D_Gain").values[0]
        i_gain = self.srv_get_register(cmd=RegisterValuesRequest.SINGLE_MOTOR, motor_name=joint_name, addr_name="Position_I_Gain").values[0]
        return [p_gain, i_gain, d_gain]

    ### @brief Helper function to set the position PID parameters for the specified joint
    ### @param joint_name - name of joint to set gains for
    ### @param pid_values - a vector of length 3 containing the desired Kp, Ki, and Kd (respectively)
    ### @details PID gains are in arbitrary units between 0 and 16,383.  Default [kp,ki,kd] is [100,0,0]
    def set_joint_position_pid_params(self, joint_name, pid_values):
        assert(len(pid_values) == 3)
